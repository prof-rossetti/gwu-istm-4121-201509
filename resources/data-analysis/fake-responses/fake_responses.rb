...
# NOTE: this script is not optimized for performance, and may take several moments to execute.
...
def generate_responses(number_of_rows)
  puts "GENERATING #{number_of_rows} ROWS OF RESPONSE DATA ..."
...
#
    ...
    fake_responses << response_attributes
  end
...
def write_responses_to_file(responses)
  ...
  FileUtils.rm_f(fake_responses_path)

  CSV.open(fake_responses_path, "w", :write_headers=> true, :headers => responses.first.keys.map{|k| k.to_s}) do |csv|
    responses.each do |response_attributes|
      csv << response_attributes.values
    end
  end
end

[1_000,10_000,100_000].each do |n|
  fake_responses = generate_responses(n)
